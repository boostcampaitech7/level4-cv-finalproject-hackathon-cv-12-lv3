        ...
        for domain, question, target_answer, target_file_name in zip(domains, questions, target_answers, target_file_names):
            ...
            try:
                ...
                if response:
                    ...
                    # 답변 평가
                    evaluation_results = llm_evaluate([question], [gen_answer], [target_answer], openai_api_key, anthropic_api_key)
                    print(f"평가 결과: {evaluation_results[0]}")

                    # 도메인별 및 전체 정답 수 카운트
                    if evaluation_results[0] == "O":
                    ...
                    for domain, stats in domain_stats.items():
                        ...
                        print(f"    질문 수: {stats['total']}")
                        print(f"    정답 수: {stats['correct']}")
                        print(f"    정확도: {(stats['correct'] / stats['total']) * 100:.2f}%")
