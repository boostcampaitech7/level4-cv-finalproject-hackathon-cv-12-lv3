...
def llm_evaluate(questions: list, generated_answers: list, target_answers: list, openai_api_key: str, anthropic_api_key: str) -> list:
    ...
    def tonic_validate(questions: list, generated_answers: list, target_answers: list, model: str) -> list:
        ...
        for idx, (question, target_answer, generated_answer) in enumerate(zip(tqdm(questions), target_answers, generated_answers)):
            try:
                ...
                eval_check.append(-1)
                print(f"Tonic {idx + 1}: -1 (Exception)")
        return eval_check
    ...
    def allganize_eval(questions: list, generated_answers: list, target_answers: list, model: str) -> list:
        ...
        for idx, (question, target_answer, generated_answer) in enumerate(zip(tqdm(questions), target_answers, generated_answers)):
            try:
                ...
                logger.warning(f"llm_eval exception: {e}")
                eval_check.append(-1)
                print(f"Allganize {idx + 1}: -1 (Exception)")
        return eval_check
    ...
    def mlflow_eval(question_list: list, answer_list: list, ground_truth_list: list, model: str):
        ...
        #     print(f"MLflow Similarity {idx + 1}: {score}")
        for idx, score in enumerate(mlflow_answer_correctness):
            print(f"MLflow Correctness {idx + 1}: {score}")

        return mlflow_answer_correctness
    ...
    def eval_vote(
        ...
        for idx in range(len(tonic_answer_similarity)):
            ...
            print(f"Final Evaluation {idx + 1}: {result}")
            e2e_result.append(result)
