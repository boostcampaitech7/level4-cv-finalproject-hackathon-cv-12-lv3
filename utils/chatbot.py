...
def is_follow_up_request(query: str, model) -> bool:
    ...
    pattern_vectors = np.array([model.encode(pattern)
...
def is_paper_info_request(query: str, model) -> bool:
    ...
    query_vector = np.array(model.encode(query)).reshape(1, -1)
    pattern_vectors = np.array([model.encode(pattern)
                               for pattern in paper_info_patterns])
...
def query_and_respond_reranker_compare(query: str, conn, model, reranker_model, user_id,
    ...
    try:
        ...
        matches_reranked = rerank_with_cross_encoder(
            query, matches_bge, reranker_model, top_k=top_k)
        ...
        for match in matches_bge[:top_k]:
            print(
                f"Page {match['page']}: {match['text']}/n(Score: {match['score']:.4f})")
